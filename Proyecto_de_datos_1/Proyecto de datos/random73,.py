...
overall_mean = np.mean([mean_cabello, mean_alameda, mean_republica, mean_sanjuan])
...
mape_cabello = mean_absolute_percentage_error(cabello_original, overall_mean)
mad_cabello = mean_absolute_deviation(cabello_original, overall_mean)
print(f"CABELLO - MAPE: {mape_cabello:.2f}%, MAD: {mad_cabello:.2f}")

# Calcular MAPE y MAD para Alameda
mape_alameda = mean_absolute_percentage_error(alameda_original, overall_mean)
mad_alameda = mean_absolute_deviation(alameda_original, overall_mean)
print(f"ALAMEDA - MAPE: {mape_alameda:.2f}%, MAD: {mad_alameda:.2f}")

# Calcular MAPE y MAD para República de Chile
mape_republica = mean_absolute_percentage_error(republica_original, overall_mean)
mad_republica = mean_absolute_deviation(republica_original,overall_mean)
print(f"REPÚBLICA DE CHILE - MAPE: {mape_republica:.2f}%, MAD: {mad_republica:.2f}")

# Calcular MAPE y MAD para San Juan
mape_sanjuan = mean_absolute_percentage_error(sanjuan_original, overall_mean)
mad_sanjuan = mean_absolute_deviation(sanjuan_original, overall_mean)
