...
Cabello_pred = pd.read_excel('predicciones_cabello_alameda.xlsx')
Cabello_pred = Cabello_pred.rename(columns={'Time': 'ds', 'Cabello_predicho': 'y'})
...
merged_data = pd.merge(Cabello_pred, maximos_minimos, on='ds')
...
merged_data['abs_diff'] = abs(merged_data['y'] - forecast['yhat'][:len(merged_data)])
