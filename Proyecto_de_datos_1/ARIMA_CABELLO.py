...
print(data_cabello.head())

# Visualizar los datos
data_cabello.plot()
plt.title('Conteo de Bicicletas en Cabello')
plt.ylabel('Número de Bicicletas')
plt.xlabel('Fecha')
plt.show()

# Buscar los mejores parámetros (p, d, q)
modelo = pm.auto_arima(data_cabello, seasonal=False, trace=True)
...
p, d, q = modelo.order
model = ARIMA(data_cabello, order=(p, d, q))
model_fit = model.fit()
...
# Hacer predicciones
predicciones = model_fit.predict(start=len(data_cabello), end=len(data_cabello) + 77)
print(predicciones)

# Visualizar las predicciones
plt.figure(figsize=(10, 6))
plt.plot(data_cabello, label='Histórico')
...
data_cabello = merged_data['Cabello_predicho']
exog = merged_data[['Temp. Máxima', 'Temp. Mínima']] 

print(merged_data.head())

# Buscar los mejores parámetros (p, d, q) con variables exógenas
modelo = pm.auto_arima(data_cabello, exogenous=exog, seasonal=False, trace=True)
print(modelo.summary())

# Ajustar el modelo ARIMA con variables exógenas
p, d, q = modelo.order
model = ARIMA(data_cabello, exog=exog, order=(p, d, q))
model_fit = model.fit()
...
# Hacer predicciones
predicciones = model_fit.predict(start=len(data_cabello), end=len(data_cabello) + 77, exog=exog_future)
print(predicciones)
...
plt.plot(data_cabello, label='Histórico')
plt.plot(predicciones, label='Predicción', color='red')
plt.title('Predicción de Conteo de Bicicletas en Cabello con Variables Exógenas')
plt.ylabel('Número de Bicicletas')
plt.xlabel('Fecha')
plt.legend()
plt.show()
