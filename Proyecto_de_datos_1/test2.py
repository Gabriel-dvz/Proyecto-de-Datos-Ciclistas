...
X = datos[['Cabello', 'República de Chile con San Joaquín', 'San Juan con Escrivá de Balaguer']].copy()
y = datos['Alameda'].copy()
...
datos_completos['Alameda_predicho'] = modelo.predict(X)

# Mostrar los datos originales y los datos completados
print("Datos originales:")
print(datos_completos[['Time', 'Alameda', 'Alameda_predicho']])
...
ax.plot(datos_completos['Time'], datos_completos['Alameda'], label='Alameda (original)', marker='o')
ax.plot(datos_completos['Time'], datos_completos['Alameda_predicho'], label='Alameda (predicho)', linestyle='--')

ax.set_xlabel('Fecha')
ax.set_ylabel('Cantidad de personas')
ax.set_title('Regresión lineal para completar datos faltantes en Alameda')
ax.legend()
ax.grid(True)
