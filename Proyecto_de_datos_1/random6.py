...
data = pd.read_excel('predicciones_cabello_alameda.xlsx')

# Asumimos que los datos tienen una columna llamada 'Time' y otra 'Alameda_predicho'
...
model_simple = ExponentialSmoothing(data_series, trend=None, seasonal=None)
...
model_trend = ExponentialSmoothing(data_series, trend='add', seasonal=None)
...
# Concatenar la serie original con las predicciones futuras
full_forecast = pd.concat([data_forecast_trend_seasonal, data_forecast_future])

plt.figure(figsize=(10, 6))
plt.plot(data_series, label='Original')
plt.plot(full_forecast, label='Forecast', color='red')
