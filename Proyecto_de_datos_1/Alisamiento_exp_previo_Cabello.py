...
#Descomposición 
decomposition = sm.tsa.seasonal_decompose(series['Cabello_predicho'], period=12) 
figure = decomposition.plot()
plt.show()

#Descomposición con additive 
decomposition = sm.tsa.seasonal_decompose(series['Cabello_predicho'], period=12, model='additive') 
figure = decomposition.plot()
plt.show()

#Histograma 

series['Cabello_predicho'].plot.hist(bins=25, alpha=0.5)
plt.show()
...
X = series.Cabello_predicho.values
split = int(len(X) / 2)
