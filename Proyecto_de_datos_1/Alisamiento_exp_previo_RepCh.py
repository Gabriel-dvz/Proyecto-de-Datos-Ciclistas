...
series = pd.read_excel('predicciones.xlsx')
...
#Descomposición 
decomposition = sm.tsa.seasonal_decompose(series['República de Chile_predicho'], period=12) 
figure = decomposition.plot()
plt.show()

#Descomposición con additive 
decomposition = sm.tsa.seasonal_decompose(series['República de Chile_predicho'], period=12, model='additive') 
figure = decomposition.plot()
plt.show()

#Histograma 

series['República de Chile_predicho'].plot.hist(bins=25, alpha=0.5)
plt.show()
...
X = series['República de Chile_predicho'].values
split = int(len(X) / 2)
